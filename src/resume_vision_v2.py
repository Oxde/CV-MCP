#!/usr/bin/env python3
"""
Resume Vision MCP Server V2 - Production Version
=================================================
🚀 CURRENT PRODUCTION VERSION with optimized PDF generation

Features:
- FastMCP framework for stability (vs V1's basic approach)
- Optimized PDF generation with Playwright
- Graceful fallback to original PDF exporter
- Professional single-page PDFs (~183KB)
- No unwanted headers/footers
- Lazy loading for optimal performance

Uses the same stable FastMCP framework as V1 but integrates our discovered 
optimal PDF settings from extensive testing of 6+ generation methods.
"""
...
# Create FastMCP server (same as working final version)
mcp = FastMCP("resume-vision", 
              description="MCP server for CV/resume editing workflow with optimized PDF generation. "
                          "Upload documents → AI vision analysis → HTML editing → Optimized PDF export. "
                          "Works with Cursor conversation flow - no external API keys needed!")

# Set explicit project directory (same as working final version)
PROJECT_DIR = Path("/Users/nikmarf/JobKiller")
WORKSPACE_DIR = PROJECT_DIR / "resume_workspace"

logger.info("🎯 Resume Vision MCP V2 initialized with FastMCP and optimized PDF generation")

# Component cache and session tracking (same as working final version)
_component_cache = {}
current_session = {
...
def ensure_workspace():
    """Ensure workspace directories exist (same as working final version)"""
    try:
...
def get_component(component_name: str):
    """Lazy load components only when needed (same as working final version)"""
    ...
    try:
        ...
        if component_name == "document_converter":
            ...
            # Try to load optimized PDF exporter first, fallback to original
            try:
                module = importlib.import_module("optimal_pdf_exporter")
                component = module.OptimalPDFExporter(str(WORKSPACE_DIR / "pdf"))
                _component_cache[component_name] = component
                logger.info("✅ OptimalPDFExporter loaded (using optimized settings)")
                return component
            except Exception as e:
                logger.warning(f"⚠️  OptimalPDFExporter failed, falling back to original: {e}")
                # Fallback to original PDF exporter
                module = importlib.import_module("pdf_exporter")
                component = module.PDFExporter(output_dir=str(WORKSPACE_DIR / "pdf"))
                _component_cache[component_name] = component
                logger.info("✅ PDFExporter loaded (original version)")
                return component
...
# Resource for getting vision prompt (same as working final version)
...
# Document Processing Tools (same as working final version)
@mcp.tool()
    ...
    try:
        ...
        return {
            ...
            "next_options": [
                ...
                "@export_to_pdf - Create optimized PDF"
            ]
        }
...
# Enhanced PDF export with optimized settings
@mcp.tool()
def export_to_pdf(html_file_path: str, output_name: Optional[str] = None) -> Dict[str, Any]:
    """Export HTML resume to PDF using optimized generation settings."""
    ...
    try:
        logger.info(f"📄 Exporting to PDF with optimized settings: {html_file_path}")
        
        # Check if we have the optimized exporter
        if hasattr(pdf_exporter, 'convert_html_to_pdf') and 'OptimalPDFExporter' in str(type(pdf_exporter)):
            # Use optimized exporter
            result = pdf_exporter.convert_html_to_pdf(html_file_path, output_name)
            ...
            if Path(result).exists():
                ...
                return {
                    "success": True,
                    "pdf_path": result,
                    "message": f"✅ PDF exported with optimized settings: {Path(result).name}",
                    "file_size_kb": round(file_size / 1024, 1),
                    "configuration": "Optimized (Playwright, 0.6in margins, 0.85 scale, single-page)"
                }
        else:
            # Use original exporter
            ...
            if result.endswith('.pdf') and Path(result).exists():
                ...
                return {
                    ...
                    "configuration": "Standard PDF generation"
                }
            else:
...
# Workflow and status tools (same as working final version)
@mcp.tool()
def get_workflow_status() -> Dict[str, Any]:
    ...
    for comp_name in ["document_converter", "vision_replicator", "template_manager", "ai_editor", "pdf_exporter"]:
        if comp_name in _component_cache:
            if _component_cache[comp_name]:
                if comp_name == "pdf_exporter" and 'OptimalPDFExporter' in str(type(_component_cache[comp_name])):
                    component_status[comp_name] = "✅ loaded (optimized)"
                else:
                    component_status[comp_name] = "✅ loaded"
            else:
                component_status[comp_name] = "❌ failed"
        else:
    ...
    return {
        ...
        "message": "🎯 Resume Vision MCP is operational with optimized PDF generation"
    }
...
# Helper functions (same as working final version)
...
if __name__ == "__main__":
    try:
        logger.info("🚀 Starting Resume Vision MCP Server (Optimized Fixed)...")
        mcp.run()
